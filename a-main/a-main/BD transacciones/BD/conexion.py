...
class DAO():    # acrónimo de Data Access Object
    ...
    def resumenMensual(self,mes):
        if self.conexion.is_connected():
            try:
                cursor=self.conexion.cursor()
                cursor.execute("SELECT fecha,monto FROM transacciones WHERE fecha like '%/{0}/%'".format(mes))
                resultado=cursor.fetchone()
                return resultado
            except Error as ex:
                print("Error de conexión: {0} ".format(ex))

    def buscarTodasTransacciones(self,mes):
        if self.conexion.is_connected():
            try:
                cursor=self.conexion.cursor()
                cursor.execute("SELECT monto FROM transacciones WHERE fecha like '%/{0}/%'".format(mes))
                resultado=cursor.fetchall()
                return resultado
            except Error as ex:
                print("Error de conexión: {0} ".format(ex))
